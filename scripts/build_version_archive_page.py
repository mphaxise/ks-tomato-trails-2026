#!/usr/bin/env python3
"""Build deployable tracker version archive page and copy release tracker snapshots.

Cloudflare Pages deploys only the `tracker/` directory. This script mirrors
`releases/<version>/tracker/*` into `tracker/releases/<version>/tracker/*`
and builds `tracker/version-archive.html` with links that resolve within that
published root.
"""
...
RELEASES_SOURCE_ROOT = Path("releases")
RELEASES_DEPLOY_ROOT = TRACKER_ROOT / "releases"
VERSION_ARCHIVE_PATH = TRACKER_ROOT / "version-archive.html"
...
def normalize_versions(manifest: Dict[str, object]) -> List[Dict[str, object]]:
    ...
    versions.sort(key=lambda item: str(item.get("release_date", "")))
    return versions


def copied_set(version: Dict[str, object]) -> Set[str]:
...
def link_entries(copied: Set[str], base_url: str) -> Sequence[str]:
    ordered = [
...
def sync_release_tracker_snapshots(versions: Sequence[Dict[str, object]]) -> int:
    if RELEASES_DEPLOY_ROOT.exists():
        shutil.rmtree(RELEASES_DEPLOY_ROOT)
    RELEASES_DEPLOY_ROOT.mkdir(parents=True, exist_ok=True)
    ...
    for version in versions:
        ...
        if not version_id:
            continue
        source_dir = RELEASES_SOURCE_ROOT / version_id / "tracker"
        target_dir = RELEASES_DEPLOY_ROOT / version_id / "tracker"
...
def main() -> int:
    ...
    copied_versions = sync_release_tracker_snapshots(versions)

    html = build_html(versions)
    VERSION_ARCHIVE_PATH.write_text(html, encoding="utf-8")

    print(f"manifest={MANIFEST_PATH}")
    print(f"versions={len(versions)}")
    print(f"copied_release_trackers={copied_versions}")
    print(f"deploy_release_root={RELEASES_DEPLOY_ROOT}")
    print(f"archive_html={VERSION_ARCHIVE_PATH}")
    return 0
